...
from utils.demucs.demuscutils import center_trim, unfold, spectro, ispectro
...
class DConv(nn.Module):
    ...
    def __init__(self, channels: int, compress: float = 4, depth: int = 2, init: float = 1e-4,
                 norm=True, attn=True, heads=4, ndecay=4, lstm=True, gelu=True,
...
class Demucs(nn.Module):
    ...
    def __init__(self,
        ...
        self.lstm = BLSTM2(chin, bi=not causal)
        if rescale:
...
class HDemucs(nn.Module):
    ...
    def __init__(self,
        ...
        for index in range(depth):
            ...
            # TODO do henclayer dodac adain speaker encoder
            if multi:
    ...
    def forward(self, mix):
        ...
        if self.hybrid:
            xt = xt.view(B, 1, -1, length)
            xt = xt * stdt[:, None] + meant[:, None]
            x = xt + x
        return x.view(B, -1, length)